...
| --- | --- |
| 高阶 skill 仓库的深度拆解 | G1 |
| 评测工具链怎么接入 skill 闭环 | G2 |
| 回归 Harness 的工程规格 | G3 |
| 生态效果的清醒证据 | G4 |
| Skill 过多时的编排与 recall | G5 |
| 跨 Surface 运行时深水区 | G6 |

主文和附录 A-F 覆盖了从"怎么读样本"到"怎么搭闭环"的完整学习路径，但为了让那条路径更容易爬，有些高阶话题被有意压轻了。这份附录把它们集中呈现，适合以下读者：
...
**评测 harness**：这是 gstack 与其他仓库最大的分水岭。它有完整的 E2E 评测基础设施——LLM-as-judge 评分、多 surface session runner、结构化 trace 存储、before/after 对比。后面 G3 节会详细讲这套 harness。
...
> **前置条件：** 你已经理解 G2 中的工具思路，现在想知道"如果我要自己搭一套 skill 回归测试系统，架构应该长什么样"。
...
这不是说公开 skill 没有学习价值——G1 推荐的四个仓库正是很好的学习对象。但它提醒我们：**"共享"不等于"对你的任务有帮助"**。采用任何公开 skill 都需要在你自己的代表性任务上做验证，而不是看到 star 数高就直接用。
...
2. **Learning（学习）**：它的设计有没有值得学的地方。G1 推荐的四个仓库优先满足这层。
...
这意味着同一个 skill 包，在不同 surface 上的安装位置和发现机制不同。如果你要做多 surface 分发，需要为每个 surface 写安装说明或安装脚本——superpowers 在这方面做了很好的示范（参见 G1 中对 superpowers 适配层的描述）。
...
如果你已经看完了主文和前五个附录再来看这份附录 G，说明你对 skill engineering 的认识已经远超"会写一个 SKILL.md"的阶段了。

回到主文结尾的成熟度阶梯：

- G1 的仓库阅读让你看到 **Level 4-5** 的实践者在做什么
- G2-G3 的工具链和 harness 是 **Level 5**（有优化闭环）的基础设施
- G4 的生态判断让你在做采用决策时不再只看 star 数
- G5-G6 的编排和运行时知识是 skill 从"个人使用"走向"团队推广"的必备认知
