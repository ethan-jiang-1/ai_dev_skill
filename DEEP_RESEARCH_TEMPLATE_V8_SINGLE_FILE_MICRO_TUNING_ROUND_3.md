...
> document_role: `Round 3 评估 + 改动控制稿`
...
1. 先审 `Plan Skeleton` 中带有状态感的块，只处理 `Wave Gate Scoreboard`、`Current Topology` 与相关短导语。
2. 再审 `Status File Skeleton` 中带有结构规范感的块，只处理 `当前结论 / 进度语义`、`当前拓扑与 Formalization State`、`Resume Checkpoint`。
3. 做一次交叉核对，确认：
   - `Plan` 里不再承担动态工作台职责
   - `Status` 里不再承担结构蓝图职责
