...
- `REFERENCE_DIR` 的完成单位不是“看过这个链接”，而是“这个链接已经被整理成可复用、可回指的独立 `md` 文档”
...
- 以“reference 独立成文”为纪律：每个关键来源单独存成一份 `md`，不要把辛苦收集的内容只留在临时笔记或上下文中
...
- 每个关键来源都已经在 `<REFERENCE_DIR>` 中独立存成 `md`
