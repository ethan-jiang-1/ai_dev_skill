...
这段记录的是早期过渡判断，不是最终架构承诺。

- 早期判断曾考虑把显性队列先放进 `STATUS_PATH`
- 后续在本控制稿 6.1-6.5 中已明确收口为：升格到独立 `QUEUE_PATH`
- 因此这里保留的价值，只是解释为什么当时会先从 `Active Todo Queue` 这个概念出发

最终强约束以 `QUEUE_PATH` 方案为准，不再要求在 `STATUS_PATH` 内维护完整 active queue。
...
- `STATUS_PATH`
  ...
  - 当 `queue_health = blocked` 时，同步：`state = blocked`、`blocking_issue`、`safe_to_interrupt`
  - `Resume Checkpoint` 只保留恢复指针与 precheck，不复制 active queue
...
- `current_task` 必须指向下一位执行者的首个未完成动作，不能回填已经完成的 instantiation 步骤
- 当 `queue_health = blocked` 时，必须显式写出 `blocked_reason / interrupt_condition_matched / unblock_trigger`
- 当 `queue_health = blocked` 时，`STATUS_PATH` 也必须同步进入 blocked 语义，不能和 queue 打架
...
- `plan/status/queue` 复制边界不再依赖松散 prose，而是依赖 markers
...
- `QUEUE_PATH` 成为默认“接下来干什么”的显性入口
- `Resume Checkpoint` 不再偷偷维护第二套 next-step queue
- `QUEUE_PATH` 的 blocked 状态与 refill 结构对 LLM 是显性、可填、可恢复的
...
5. 把 `QUEUE_PATH` 三件套改造完整登记到本控制稿。
6. 把 `STATUS_PATH` 中的 queue 影子字段收缩为摘要与恢复指针。
