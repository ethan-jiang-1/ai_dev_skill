...
- 无论目录放在哪里，都必须保证接手者能在 30 秒内定位到本轮 reference 与 artifacts
...
- `<REFERENCE_DIR>` 与 `<ARTIFACT_DIR>` 已经先被初始化，而不是边搜边临时决定放哪
...
这一节是 V5 新增的核心协议，目的是让长程研究任务真正做到自主推进、最少干扰用户。
...
- V4 的 human-on-the-loop 原则保留不变
- V5 进一步明确：on-the-loop 的"loop"粒度是 wave 级别或 closeout 级别，不是 step 级别
...
2. 初始化 `<REFERENCE_DIR> / <REFERENCE_DIR>/_INDEX.md / <ARTIFACT_DIR>`，先建可回指入口再写结论。
...
- 每轮都必须能做到 30 秒本地回指
- 每轮都必须默认自主推进，只在主线阻塞或方向根本性调整时才中断用户；进度靠 status 文件传递，不靠对话汇报
