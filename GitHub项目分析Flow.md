...
为了避免这个 flow 越做越散，默认按 phase 推进，  
不要在前一个 phase 还没站稳时，提前跳到后一个 phase。

建议固定按下面五个 phase 走：

- **Phase 1：建骨架**  
  目标：把 analysis 目录、来源记录、snapshot 放对位置
- **Phase 2：做盘点**  
  目标：先弄清楚仓库类型、关键目录、skill-like 单元分布
- **Phase 3：做理解**  
  目标：把主流程、协作关系、验证机制、可复用点看明白
- **Phase 4：做评估**  
  目标：在前面事实充分的前提下，进入 eval_skills 最终产出
- **Phase 5：做补充**  
  目标：只在必要时补多模型版本或横向对比

phase 之间的执行纪律：

- 前一个 phase 没完成，不抢跑下一个 phase
- 如果发现前面信息不够，允许回退，不允许硬评
- eval_skills 默认只在 Phase 4 才开始集中产出
- 这样做的目的就是：**步步为营，尽量不跑偏**
...
从 Step 3 到 Step 6，核心还是在做 **Phase 2：做盘点**。
