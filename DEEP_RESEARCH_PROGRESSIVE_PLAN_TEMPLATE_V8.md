...
| `B. Instantiation Output Boundary` | 定义实例化产物的最小继承规则、skeleton、初始化快照 | 生成或修复 `plan + status + queue` 时读 | `plan/status/queue skeleton` only |
| `C. Execution Protocol` | 定义执行期推进方式、`QUEUE_PATH`、分支处置、证据采集与并行建议 | 明确进入 `Execution Mode` 后读 | `no`；execution-only |
...
| --- | --- | --- | --- | --- |
| `A. Routing + Core Contract` | `no` | `no` | `no` | 只允许白名单最小规则通过 `Inherited Minimum Rules` 继承 |
| `B. Inherited Minimum Rules for Instantiated Plan` | `yes` | `no` | `no` | 只继承最小边界规则，不复制整块控制层 |
...
| --- | --- | --- | --- |
| `Instantiation Mode` | 只生成或修复 `plan + status + queue` | `current_gate = instantiation_complete` | 目录已初始化或 `Wave 0` 已开始 |
...
8. 初始化 queue 的最小字段（写入下一位执行者的首个未完成动作，不要回填已经完成的 instantiation 动作）：
   ...
   - `queue_health = ready`
9. 完成实例化检查后停止，不自动进入目录初始化与 Wave 执行。
...
- 如果 queue 已经开始记录 execution-only 的 live branch routing，也说明已经越界进入执行态
- 在 `Instantiation Mode` 中，允许写入的是：实例参数、topic registry、topology baseline、core gaps、初始 gate state、queue seed、resume entry
...
- queue 文件已经存在，并包含连续可执行队列
- queue 的 `current_task` 指向下一位执行者的首个未完成动作，而不是已完成的 instantiation 步骤

#### B. Execution Mode

1. 再次确认 `PLAN_PATH`、`STATUS_PATH` 与 `QUEUE_PATH` 已存在，且实例化检查已经通过。
...
6. 进度、缺口、挂起分支、失败探索和恢复入口写入 status；连续动作写入 queue；不要留在聊天上下文里。
...
- `STATUS_PATH` 的初始目标是“给执行者一个稳定状态起点”，不是“提前汇报 Wave 进度”
- `QUEUE_PATH` 的初始目标是“给执行者一个连续动作起点”，不是“写完整项目计划”
- `QUEUE_PATH` 的初始 active queue 必须指向实例化完成后的第一个未完成动作；不要把已完成的实例化步骤写回 `current_task`
...
- 初始 status 与初始 queue 一律优先采用模板提供的推荐初始化值，除非当前任务明确要求直接进入执行态
...
| `<STATUS_PATH>` | `run-time Source of Record`；当前状态、当前 gate、当前缺口、阻塞、挂起分支、恢复入口 |
| `<QUEUE_PATH>` | `authoritative execution queue`；当前动作、后续动作、queue refill、promotion 规则 |
...
| `queue_health` | queue status field | 不写完整 queue 内容 |
| `current_task_summary` | short action summary | 只写当前动作摘要 |
...
- recommended_resume_entry: `read Instance Config -> queue_path -> Gate State`

- queue_path: `<QUEUE_PATH>`
- queue_health: `ready`
- current_task_summary: `initialize execution surfaces and verify directory scaffolding`
...
- recommended_resume_entry: `read <QUEUE_PATH> -> Gate State -> Wave 0`

- queue_path: `<QUEUE_PATH>`
- queue_health: `ready`
- current_task_summary: `land next high-value shared reference`
...
- 生成 `status` 与 `queue` 时，默认只按 skeleton 与推荐初始化快照落盘
...
- **MUST**：`required_next_step`、`current_task_summary` 与 `Resume Checkpoint` 只保留摘要、恢复指针或 precheck；不要复制 `QUEUE_PATH.Active Queue`
