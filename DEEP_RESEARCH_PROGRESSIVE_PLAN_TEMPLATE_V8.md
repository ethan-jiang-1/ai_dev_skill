...
| `primary_source_status` | `dimension-status` | `STATUS_PATH -> Wave 1` | 只描述 primary source 维度状态；不替代研究线级 `topic_stop_decision` |
...
这一节只起白名单约束作用，不单独复制进实例 plan。

- 对象边界统一回读 `Structural Spine`
- 枚举和值域统一回读 `Canonical Enum Registry`
- 字段角色统一回读 `Canonical Field Role Notes`
- 运行期 authority 统一回读 `Canonical Runtime Authority`
...
- `topic_stop_decision` 是研究线级决策；`primary_source_status` 只描述 primary source 维度状态
