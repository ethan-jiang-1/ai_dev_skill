...
| `topic_stop_decision` | `continue / early_saturation / suspend / archive / redirect` | 只用于 Wave 1 研究线级处置 |
...
| `topic_stop_decision` | `decision-state` | `STATUS_PATH -> Wave 1` | `continue` 留在主线记录；非主线处置可映射到 `Suspended Branches` |
...
| `topic stop decision` | `PLAN_PATH -> 搜够了没有：停止条件` | `<STATUS_PATH>.Wave 1` + `Suspended Branches` | `continue / early_saturation / suspend / archive / redirect` 都属于 topic-level decision |
...
- topic-level decision 统一使用 `continue / early_saturation / suspend / archive / redirect`
...
每条研究线在完成一轮搜集后，必须把当前状态明确归类为 `continue / early_saturation / suspend / archive / redirect` 之一。
...
| `topic_stop_decision` | topic-level decision enum | 统一使用 `continue / early_saturation / suspend / archive / redirect` |
...
- recent_count:
- limitation_count:
- topic_stop_decision: `continue / early_saturation / suspend / archive / redirect`
