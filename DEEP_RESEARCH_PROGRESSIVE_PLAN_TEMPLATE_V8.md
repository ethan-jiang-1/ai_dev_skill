...
| `B. Instantiation Output Boundary` | 实例化最小输入、最短路径、检查清单、初始化预设、skeleton | 生成或修复 `plan + status + queue` 时读 | `plan/status/queue skeleton` only |
| `C. Execution Protocol` | 执行期推进方式、`QUEUE_PATH`、分支处置、证据采集、停止与 closeout | 明确进入 `Execution Mode` 后读 | `no`；execution-only |
...
| `recommended_resume_entry` | `pointer-only` | `STATUS_PATH -> Current Execution Snapshot` | 只提供恢复入口，不写动作队列正文 |
...
| `topic stop decision` | `PLAN_PATH -> 搜够了没有：停止条件` | `<STATUS_PATH>.Wave 1` + `Suspended Branches` | `early_saturation / suspend / archive / redirect` 都属于 topic-level decision |
...
- `Wave Gate Scoreboard`：只定义 round-level progress gates，不记录当前态
- `搜够了没有：停止条件`：只定义 topic-level decision，包括 `continue / early_saturation / suspend / archive / redirect`
- `Readiness Check`：只定义 round closeout gate
- `Hard Gates`：只定义可选加严层；默认不启用
- `成功标准`：只描述通过 `Readiness Check` 后的完成态，不是额外 gate
...
| `recommended_resume_entry` | pointer-only field | 只给恢复入口，不给动作副本 |
