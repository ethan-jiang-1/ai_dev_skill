...
| --- | --- | --- | --- |
| `A. Routing + Core Contract` | 路由闸门、SSOT、术语、固定枚举、复制边界 | 进入任何 mode 之前先读 | 默认 `no`；只允许继承白名单规则 |
| `B. Instantiation Output Boundary` | 定义实例化产物的最小继承规则、skeleton、初始化快照 | 生成或修复 `plan + status` 时读 | `plan/status skeleton` only |
| `C. Execution Protocol` | 定义执行期推进方式、连续派工机制、分支处置、证据采集与并行建议 | 明确进入 `Execution Mode` 后读 | `no`；execution-only |
...
2. 在 `Instantiation Mode` 中，只复制 `BEGIN PLAN SKELETON / END PLAN SKELETON` 之间内容，以及 `BEGIN STATUS SKELETON / END STATUS SKELETON` 之间内容。
   - 先填写 `Instance Config`
...
- `Active Todo Queue`：`STATUS_PATH` 中显性维护的连续派工队列；执行期默认至少保持 `current_task / next_task / next_after_next` 三层非空
...
| `gate model` | `PLAN_PATH -> Wave Gate Scoreboard` | `STATUS_PATH` 只写 `gate state` | `PLAN_PATH` 只定义 allowed values 与默认路径 |
| `gate state / next action` | `STATUS_PATH -> Gate State` | `PLAN_PATH` 只保留 allowed values，不写当前态 | 执行推进只更新 `STATUS_PATH` |
| `active todo queue` | `STATUS_PATH -> Active Todo Queue` | 对话中可短引，不维护第二份 live queue | 每次完成当前任务前先补齐下一层队列 |
...
| section family | instantiate into `PLAN_PATH` | instantiate into `STATUS_PATH` | note |
| --- | --- | --- | --- |
| `A. Routing + Core Contract` | `no` | `no` | 只允许白名单最小规则通过 `Inherited Minimum Rules` 继承 |
| `B. Inherited Minimum Rules for Instantiated Plan` | `yes` | `no` | 只继承最小边界规则，不复制整块控制层 |
| `B. Plan Skeleton` | `yes` | `no` | 只复制 marker 内正文 |
| `B. Status File Skeleton` | `no` | `yes` | 只复制 marker 内正文 |
| `B. Recommended Initialization Snapshots` | `no` | `no` | 仅作初始化参考，不原样复制 |
| `C. Execution Protocol` | `no` | `no` | execution-only；实例 plan 只继承白名单最小规则 |
| `D. Appendix / Maintenance` | `no` | `no` | 只服务模板维护 |
...
这一块只负责“如何稳定地产出本轮 `plan + status`”。

- 谁应阅读：实例化者、修复实例化产物的人
- 何时阅读：确认进入 `Instantiation Mode` 后
- 是否进入实例产物：只允许 skeleton 与白名单最小规则进入
...
### Inherited Minimum Rules for Instantiated Plan（实例 plan 必须继承的最小规则）

这一节是实例 plan 允许从模板控制层继承的最小白名单。

- `<PLAN_PATH>` 是本轮 `design-time Source of Record`；不写实时进度
- `<STATUS_PATH>` 是本轮 `run-time Source of Record`；当前 gate、当前缺口、当前 next step 都写这里
...
- 实例化产物默认只复制 marker 内的 skeleton 正文；section 级许可统一以 `Instantiation Copy Boundary` 为准
...
- 枚举和值域统一以 `Canonical Enum Registry` 与 skeleton 字段注释为准；不要自造第三套状态名
...
这是一组执行前置约束，用于指导进入 `Execution Mode` 后的目录初始化；默认不原样复制进实例产物。
...
| `Active Todo Queue.*` | explicit queue entry | 任何时刻至少保持 `current_task / next_task / next_after_next` 非空，除非满足允许中断条件 |
...
- queue_maintenance_rule: `before closing current_task, refill the queue so that the next visible actionable item never becomes empty`
...
- current_task: `instantiate plan and status skeletons`
- next_task: `clear placeholders or mark them as 暂按假设执行`
- next_after_next: `stop at instantiation_complete without entering execution`
- queue_health: `ready`
...
- current_task: `land next high-value shared reference`
- next_task: `update _INDEX and directory integration state`
- next_after_next: `backfill relevant seed or artifact before moving on`
- queue_health: `ready`
...
#### 1.1 Active Todo Queue（连续派工机制，MUST）

本模板明确要求：执行期不能让模型进入“当前任务做完，但接下来做什么暂时为空”的状态。

- **MUST**：把连续派工显式写入 `<STATUS_PATH>.Active Todo Queue`
- **MUST**：默认至少保持下面 3 层非空：
  - `current_task`
  - `next_task`
  - `next_after_next`
...
- **MUST**：把“下一步在哪看”设计成显式可检索入口；默认入口就是 `<STATUS_PATH>.Active Todo Queue`
- **MUST NOT**：让 `required_next_step` 成为唯一下一步字段；它是当前单步要求，不替代显性待办队列
- **MUST NOT**：在没有满足允许中断条件时，把 `Active Todo Queue` 留空
...
显式待办队列的目标不是做完整 project plan，而是避免 agent 在局部完成后获得“喘气机会”并错误停机。
...
- `<STATUS_PATH>.Active Todo Queue` 是默认“接下来干什么”的显性入口
- Worklog 必须持续追加，记录关键推进和关键判断
- 用户主动询问进度时可以简要回答，但默认不主动发起进度汇报
- 如果用户中途插入其他任务，恢复时优先读 `<STATUS_PATH>` 的当前状态、Active Todo Queue、Gate State、Suspended Branches、Failed Explorations 和 Resume Checkpoint
...
- 以“显性待办队列永不见底”防止中途停机：任何时刻都要让 `Active Todo Queue` 保持至少 3 层可见动作，除非主线真正阻塞
