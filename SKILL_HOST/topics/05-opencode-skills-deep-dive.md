...
- OpenCode 的官方 docs 已经非常明确地把“桥接能力”写在产品结构里：
  - skills 可兼容 `.opencode`、`.claude`、`.agents` 多路径，而且官方明确写出扫描路径与 discovery 规则（walk up 到 git worktree）[ref](./_reference/05-opencode-agent-skills-discovery-and-compatibility-paths.md)
  - 同时 OpenCode 还明确了 frontmatter schema 与 name regex（`^[a-z0-9]+(-[a-z0-9]+)*$`），这意味着“能兼容”也会带来更强的规范化要求 [ref](./_reference/05-opencode-agent-skills-discovery-and-compatibility-paths.md)
...
- OpenCode 的模型面非常宽：
  - `75+` providers
  - local models
  - `/connect` + `~/.local/share/opencode/auth.json` 把 provider onboarding 做成显式流程与本地凭据落点
  - `baseURL` override 支持代理/自建 endpoint
  - OpenCode Zen/Go 作为被测试验证过的 model 集合 [ref](./_reference/05-opencode-providers-connect-auth-and-credential-storage.md)
  - `/models` 入口
