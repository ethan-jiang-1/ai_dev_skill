...
- Codex 的“可治理性”不仅写在概念里，还被做成了可配置的 contract：
  - `approval_policy` 支持 granular 政策，且 `on-failure` 明确被弃用
  - `skills.config` 支持 per-skill enable/disable
  - `allowed_web_search_modes` 明确把 `disabled/cached/live` 变成可治理开关
  - `tools.web_search` 支持 domains/location/context size 这类“检索语义配置”
  - `mcp_servers` 允许通过 allowlist + identity match 关闭不受信任 MCP [ref](./_reference/03-codex-config-reference-policy-and-skill-controls.md)
...
- 而且 worktrees 不是 changelog 里的“名词”，而是有独立官方执行拓扑说明：
  - Git repo 下 automations 在 dedicated background worktrees 上跑
  - non-git 项目则直接在 project directory 跑
  - `Handoff` 允许把 thread 在 `Local <-> Worktree` 间移动，并由 Codex 处理 Git 操作 [ref](./_reference/03-codex-app-worktrees-parallel-threads-and-handoff.md)
...
- Codex 对 research-heavy workflow 的执行契约也已经很具体：
  - approval policy 不是单一开关，而有 `on-request / never / untrusted / granular`
  - `workspace-write` 下 network access 独立配置
  - `web_search` 分成 `disabled / cached / live`
  - `.git / .agents / .codex` 在可写根下仍保持 protected read-only
  - subagents 会继承 parent session 的 `sandbox_mode / mcp_servers / skills.config`
  - cloud 任务中的 agent phase 默认不开互联网 [ref](./_reference/03-codex-approvals-sandbox-web-search-and-subagent-inheritance.md)
...
- 现在还可以再往下一层：Codex 不是只把“工作流组件”显式化，还把“执行治理组件”显式化：
  - approvals
  - sandbox
  - network access
  - web search mode
  - admin policy
  - subagent inheritance [ref](./_reference/03-codex-approvals-sandbox-web-search-and-subagent-inheritance.md)
- 而且这些治理组件不是“口头建议”，而是在 config reference 里被明确枚举成可配置键，并且连 web search 的 allowed modes、MCP allowlist 这种现实约束都被写进了 contract [ref](./_reference/03-codex-config-reference-policy-and-skill-controls.md)
...
- 另一个趋势是：Codex 很强调 runtime controls，而不仅是格式支持：
  ...
  - model snapshots
  - approval and web-search modes [ref](./_reference/03-codex-subagents-runtime-controls-and-cost.md) [ref](./_reference/00-shared-codex-model-requirements-and-context-windows.md) [ref](./_reference/03-codex-model-snapshots-and-cli-optimized-runtime.md) [ref](./_reference/03-codex-approvals-sandbox-web-search-and-subagent-inheritance.md)
- 难点也对应地更“工程化”：
  - subagents 增加 token 和资源消耗
  - hooks 仍然 experimental
  - research workflow 还要同时处理 approval policy、network access、web search 与 admin restrictions
...
- Codex 对运行前提的透明度也应算优势的一部分：
  - approval policy、sandbox mode、network access、web_search、admin requirements 都是显式面 [ref](./_reference/03-codex-approvals-sandbox-web-search-and-subagent-inheritance.md)
- 这意味着在 Codex 上讨论 skill，不能脱离模型、sandbox、approval policy 和 search mode 来谈；尤其是研究型或并发型 skill，更是如此。
...
- Codex 的最大优势，是它把作用域、禁用、agent schema、sandbox、approval policy、web search、reasoning effort、feature maturity 这些关键控制面都摆得比较明白。这很适合重视治理和可操作性的 CLI 用户 [ref](./_reference/03-codex-agents-md-layering-and-instruction-chain.md) [ref](./_reference/00-shared-codex-model-requirements-and-context-windows.md) [ref](./_reference/03-codex-approvals-sandbox-web-search-and-subagent-inheritance.md)
...
- Codex 的代价，是很多高阶 workflow 同样不是纯 skill 独立完成的；一旦涉及 subagents、hooks、plugins、MCP，或者 live search / network / admin policy，复杂度和资源消耗会明显上升 [ref](./_reference/03-codex-subagents-runtime-controls-and-cost.md) [ref](./_reference/03-codex-hooks-plugins-and-feature-maturity.md) [ref](./_reference/03-codex-approvals-sandbox-web-search-and-subagent-inheritance.md)
