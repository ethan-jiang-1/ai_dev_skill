...
- `../reference_src/02-dist-lobehub-market-cli-tarball-0-0-28.md`
...
- [hard_fact] Vercel Skills CLI 已出现安全治理 UX 线索：安装前可通过 audit API 拉取并展示 `Security Risk Assessments`（advisory），且 audit 失败/超时不会阻塞安装。（Ref: ../reference_src/02-dist-vercel-skills-cli-security-audit-api.md）
...
- [hard_fact] well-known 机制存在规范级演进且已被真实采用：Cloudflare RFC v0.2.0 将发现路径迁移到 `/.well-known/agent-skills/` 并引入 `$schema/type/url/digest` 与强制 digest 校验；发布者侧已出现“新旧双栈并存”的真实样本（X 与 Cognite 均同时提供 v0.2.0 与 legacy endpoints）；也存在“仅 legacy”的滞后样本（Backstage）。这解释了生态为何短期内难以口径收敛。（Ref: ../reference_src/00-shared-cloudflare-agent-skills-discovery-rfc-0-2-0.md；../reference_src/02-dist-x-well-known-agent-skills-index-json.md；../reference_src/02-dist-x-well-known-skills-index-json.md；../reference_src/02-dist-cognite-well-known-agent-skills-index-json.md；../reference_src/02-dist-cognite-well-known-skills-index-json.md；../reference_src/02-dist-backstage-well-known-skills-index-json.md）
- “规模市场”与“可信治理”是不同产品路线：LobeHub 不只停留在网页目录，其 CLI（`lhm`）已提供 `skills search/view/install` 及面向多宿主的默认落盘映射（`.claude/skills` / `.agents/skills` / `.cursor/skills` 等），并把 MCP plugins 纳入同一交互面；但版本化、筛选/审核、完整性验证与安全扫描链路仍缺一手证据。（Ref: ../reference_src/02-dist-lobehub-github-readme.md；../reference_src/02-dist-lobehub-market-cli-npm.md；../reference_src/02-dist-lobehub-market-cli-tarball-0-0-28.md）
...
- 这个主题当前的硬事实是什么：已出现至少三类分发层对象（跨宿主安装 CLI、带验证/版本治理的 hub、以及规模市场），并且安全扫描与版本快照等机制已被官方文档化；同时规模市场也开始出现可复现的 CLI 安装语义与落盘映射样本（LobeHub `lhm`）。（Ref: ../reference_src/00-shared-vercel-skills-cli-readme.md；../reference_src/02-dist-sundial-home.md；../reference_src/02-dist-sundial-docs-security.md；../reference_src/02-dist-lobehub-github-readme.md；../reference_src/02-dist-lobehub-market-cli-tarball-0-0-28.md）
...
- 采用或落地的难点在哪里：symlink vs copy 的升级治理差异、scan 覆盖面与误报/漏报、以及规模市场即便提供 CLI 入口也可能面临鉴权/凭据、版本语义与验证链路不清等落地摩擦。（Ref: ../reference_src/00-shared-vercel-skills-cli-readme.md；../reference_src/02-dist-sundial-docs-security.md；../reference_src/02-dist-lobehub-github-readme.md；../reference_src/02-dist-lobehub-market-cli-tarball-0-0-28.md）
...
- 哪些对象最值得继续追踪：Sundial（验证/版本/私有发现）、Vercel Skills（source formats + well-known registry）、LobeHub（可安装语义与 CLI 能力面），以及更多分发侧的安全与更新治理实证。（Ref: ../reference_src/02-dist-sundial-docs-security.md；../reference_src/02-dist-vercel-skills-docs-source-formats.md；../reference_src/02-dist-lobehub-market-cli-tarball-0-0-28.md；../reference_src/02-dist-lobehub-market-cli-npm.md）

## 缺口与下一步补搜

- Vercel `skills` CLI 的安全/更新治理仍缺“强信任”机制证据（例如签名/可验证来源/端到端完整性与 provenance）；目前已覆盖 lock files、hash-based update checking、update API，以及安装前的 audit advisory（安全风险提示），但仍不足以说明“来源真实性 + 端到端防篡改”的完整闭环。（Ref: ../reference_src/02-dist-vercel-skills-docs-lock-files.md；../reference_src/02-dist-vercel-skills-docs-update-system.md；../reference_src/02-dist-vercel-skills-cli-security-audit-api.md）
- well-known endpoint 已出现更多真实采用样本，但生态口径仍分裂：既存在 v0.2.0（agent-skills + digest）与 legacy（skills + files）并存（X、Cognite），也存在“发布者只做 legacy”的滞后（Backstage）；仍需补“消费侧收敛证据”（除 Vercel CLI 外，哪些安装器/宿主消费哪种 schema、以及如何处理完整性/信任）。（Ref: ../reference_src/00-shared-cloudflare-agent-skills-discovery-rfc-0-2-0.md；../reference_src/02-dist-vercel-skills-well-known-index-schema.md；../reference_src/02-dist-x-docs-skill-md.md；../reference_src/02-dist-backstage-docs-ai-skills.md；../reference_src/02-dist-cognite-well-known-agent-skills-index-json.md）
- LobeHub “skills” 的可安装语义已获得实现级证据（CLI 命令面、落盘映射、凭据读取）；但治理与信任链路仍缺一手材料：skill 标识与版本参数的真实语义、是否存在不可变版本/完整性校验、安全扫描与审核、以及与 MCP plugins 分发的治理边界。（Ref: ../reference_src/02-dist-lobehub-market-cli-tarball-0-0-28.md；../reference_src/02-dist-lobehub-github-readme.md；../reference_src/02-dist-lobehub-market-cli-npm.md）
