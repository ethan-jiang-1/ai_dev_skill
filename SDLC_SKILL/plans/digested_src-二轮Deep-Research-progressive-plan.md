...
## 当前判断（二轮综合后）
<!-- 综合第一轮和第二轮的最终判断，每条判断带 reference_src 回指 -->
```

规则：
- 第一轮摘要内容不删改，只保留
- 所有二轮新增内容进入固定章节，不混入第一轮摘要
- 每条新增关键判断都必须带本地 `reference_src/*.md` 回指
...
- “探索宽度持续增长”指：围绕同一主题，刻意扩张多个证据线与对照面（官方机制/代码契约/趋势/失败模式/社区反例），直到“核心对象清单稳定”；不是为了无限增加名字或凑配额。
- “核心对象清单稳定”指：用于最终报告推理链条的关键对象集合不再发生结构性变化；稳定后允许继续补证据链强度（机制落地细节、版本/commit、反例/局限），但不再以新增对象为主要产出。
- “不建议并行暴力搜索”与“并行推进”不矛盾：
  - 禁止的是：Wave 0 未完成就把 4 主题割裂开并行扫荡，导致重复、口径分裂与证据不可整合。
  - 允许的是：在 Wave 0 共享地基已落盘、命名分工明确后，按前缀并行补证据与回填，降低集成债。
