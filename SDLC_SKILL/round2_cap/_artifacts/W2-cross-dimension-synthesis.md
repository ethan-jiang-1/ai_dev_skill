...
- [x] 能力地图每一行评级都能给出最小证据集回指（证据不足则标注“待验证”）
  - 入口：`round2_cap/04-能力地图与迁移价值判断.md`
- [x] 能就任意 SDLC 阶段写出“失控模式 + 机制 + 实证来源”的连贯段落（至少到 01-03 维度）
  - 入口：`round2_cap/01-能力单元本质与前置规划机制.md` `round2_cap/02-构建执行与系统化调试闭环.md` `round2_cap/03-审查发布运维与状态持久化.md`
- [x] 能写出“4 个框架核心抽象异同”的横向综合判断，并明确可迁移 vs 专有实现边界
  - 入口：本文件 + `round2_cap/04-能力地图与迁移价值判断.md`
